...
import { faUsers, faCog, faChevronDown, faDownload, faFilePdf, faFileImage, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';
...
const TopBar = ({ roomData }) => {
  ...
  const domainDisplay = `milky.way/${roomData?.roomCode || ''}`;
  ...
  const handleSaveAsPng = () => {
    ...
    try {
      ...
      link.download = `milky.way-${roomData?.roomCode || 'drawing'}.png`;
  ...
  const handleSaveAsPdf = () => {
    ...
    try {
      ...
      printWindow.document.write(`
        <html>
          <head>
            <title>milky.way - ${roomData?.roomCode || 'Drawing'}</title>
            <style>
              body {
                margin: 0;
                padding: 0;
      ...
      <div className="flex items-center space-x-4">
        <div className="relative">
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700">{users?.length || 0}</span>
          </button>
                ...
                <ul className="divide-y divide-gray-100">
                  {users && users.length > 0 ? (
                    users.map(user => {
                      ...
                      const colorIndex = (user.clientID || user.name?.length || 0) % brightColors.length;
                      const avatarColor = brightColors[colorIndex];
                      
                      return (
                        <li key={user.clientID} className="flex items-center p-3">
                          <div 
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-xs">
                              {user.currentTool ? `Using: ${user.currentTool}` : 'Viewing'}
                            </p>
                          </div>
                        </li>
                      );
          ...
          {showSettingsDropdown && (
            <div className="absolute right-0 mt-2 w-60 bg-white rounded-lg shadow-xl overflow-hidden border border-gray-100">
              <div className="px-4 pt-3 pb-2">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Export</h3>
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    <FontAwesomeIcon icon={faFileImage} className="mr-2 text-blue-500" size="lg" />
                    <span>PNG Image</span>
                  </button>
                  <button 
                    className="w-full text-left px-3 py-2 text-sm text-gray-700 flex items-center bg-gray-50 rounded-md hover:bg-gray-100 transition-colors"
                  ...
                  >
                    <FontAwesomeIcon icon={faFilePdf} className="mr-2 text-red-500" size="lg" />
              ...
              <div className="px-4 pt-3 pb-2">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Canvas</h3>
              ...
              <div className="px-4 pt-3 pb-4">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Room</h3>
                <button 
